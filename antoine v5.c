...
// Configured for falling edge ONLY to fix the double-count bug:
//   Previously both the beam-break (falling) and beam-restore
//   (rising) edges each triggered the ISR, counting 2 reps per
//   movement. Falling edge only means one trigger per rep pass.
