...
#define BUTTON_PIN           GPIO_PIN_4   // PF4 (SW1) - start workout / restart
#define SW2_PIN              GPIO_PIN_0   // PF0 (SW2) - encoder confirm button
...
// WDT_RELOAD = 50,000,000 * 15 = ~15 seconds
// If the main loop stops feeding the watchdog for 15 seconds
// (e.g., user walked away mid-rest), the system resets to WORKOUT_DONE.
// -------------------------------------------------------
#define WDT_RELOAD           (50000000UL * 15) // ~15 second watchdog timeout at 50 MHz
...
//   SETUP          = 5 ms   - short poll delay; must be small so encoder
//                             transitions are not missed between loop iterations
...
// Button flags set by interrupt, consumed by main loop.
// Using flags instead of polling prevents the double-press bug:
// a polled button stays high for multiple loop iterations, firing twice.
volatile int  buttonPressed   = 0;  // SW1 (PF4) — start/restart
volatile int  confirmPressed  = 0;  // SW2 (PF0) — encoder mode confirm

// System tick counter in ms — driven by Timer0A ISR (used for rest timer)
volatile uint32_t sysTick_ms  = 0;

// Timestamp of last valid rep — used for IR lockout window in ISR
// Prevents multiple ISR fires from a single beam-break (signal bounce)
volatile uint32_t lastRepTime_ms = 0;

// How long (ms) to ignore IR triggers after a valid rep is counted.
// IR sensors bounce electrically on beam-break, firing the ISR 2-3 times
// in microseconds. A 500 ms lockout means only one rep per half-second,
// which is faster than any realistic exercise repetition rate.
#define IR_LOCKOUT_MS  500
...
// Rest time configured by potentiometer during SETUP (ms)
// Sampled live in SETUP so user can preview and adjust before starting
uint32_t          restTime_ms     = 10000; // default 10 seconds
...
char msg_rest_cfg[]   = "  Rest Time (ms): ";
char msg_enc_reps[]   = "Encoder mode: editing REPS (press SW2 to switch to SETS)\r\n";
char msg_enc_sets[]   = "Encoder mode: editing SETS (press SW2 to switch to REPS)\r\n";
...
int main(void) {
    ...
    portF_input_setup(BUTTON_PIN | SW2_PIN);         // PF4 (SW1) + PF0 (SW2) inputs
    ...
    watchdog_setup();                                // ~15 second timeout

    // GPIO interrupt: IR sensor on PE4, falling edge ONLY (active low = object detected)
    // Using falling edge only fixes the double-count bug where both the beam-break
    // (falling) and beam-restore (rising) were each triggering a rep count.
    ...
    // GPIO interrupt: SW1 (PF4) and SW2 (PF0), falling edge only.
    // Using an ISR with a flag instead of polling readButton() in the main loop
    // fixes the double-press bug: polling sees the button held high across multiple
    // loop iterations and fires the action twice. The ISR fires once per press,
    // sets a flag, and the main loop consumes the flag exactly once.
    GPIOIntTypeSet(GPIO_PORTF_BASE, BUTTON_PIN | SW2_PIN, GPIO_FALLING_EDGE);
    GPIOIntRegister(GPIO_PORTF_BASE, Button_ISR);
    GPIOIntEnable(GPIO_PORTF_BASE, BUTTON_PIN | SW2_PIN);
    ...
    while (1) {
        ...
        // --- Watchdog: only feed when a rep is detected during WORKOUT_ACTIVE ---
        // The watchdog monitors whether the user is still performing reps.
        // Feeding it every loop iteration (the previous behavior) meant it
        // never expired even if the user walked away, because the main loop
        // kept running regardless. Now it is only fed inside IR_Sensor_ISR
        // when a valid rep is counted. If no rep is detected within 15 seconds,
        // the watchdog fires and resets the system to WORKOUT_DONE.
        // During SETUP and REST_PERIOD the watchdog is fed unconditionally
        // so it does not fire while the user is legitimately configuring or resting.
        if (currentState == SETUP_STATE || currentState == REST_PERIOD_STATE ||
            currentState == WORKOUT_DONE_STATE) {
            WatchdogReloadSet(WATCHDOG0_BASE, WDT_RELOAD); // safe to feed in non-active states
        }

        // --- Sample rotary encoder and potentiometer in SETUP ---
        // Encoder sets targetReps (mode 0) or targetSets (mode 1).
        // SW2 (confirm button) switches between editing reps and sets.
        // Potentiometer sets restTime_ms so user can preview rest duration.
        if (currentState == SETUP_STATE) {

            // SW2 confirm button: toggle encoder between editing reps and sets
            if (confirmPressed) {
                confirmPressed = 0; // consume flag
                encoderMode = (encoderMode == 0) ? 1 : 0;
                if (encoderMode == 0) { printString(msg_enc_reps); }
                else                  { printString(msg_enc_sets); }
            }
            ...
            // Read potentiometer and update restTime_ms only if the value has
            // changed by more than the deadband threshold.
            // The ADC produces small fluctuations even when the knob is still,
            // so without a threshold the value prints constantly every loop.
            // A threshold of 200 ms filters out noise while still responding
            // clearly to intentional knob turns.
            #define POT_DEADBAND_MS 200
            uint32_t newRestTime = readADC_restTime();
            uint32_t diff = (newRestTime > restTime_ms) ?
                            (newRestTime - restTime_ms) :
                            (restTime_ms - newRestTime);
            if (diff > POT_DEADBAND_MS) {
                restTime_ms = newRestTime;
                printString(msg_rest_cfg);
                printInt((int)restTime_ms);
                printString(msg_newline);
            }
        }
        ...
        // SW1 button: flag set by Button_ISR, consumed once here.
        // Using a flag instead of polling readButton() prevents the double-press
        // bug where the button stays high across multiple loop iterations.
        if (buttonPressed) {
            buttonPressed = 0; // consume flag immediately
            input = IN_BUTTON;
        }
        ...
        if (nextState != currentState) {
            ...
            if (nextState == SET_COMPLETE_STATE) {
                ...
                // Use rest time already configured by potentiometer during SETUP.
                // restTime_ms was set and previewed by the user before the workout began.
                restTimer_ms    = restTime_ms;
                restTimerActive = 1;
            ...
            if (nextState == SETUP_STATE) {
                ...
                setsCompleted  = 0;
                encoderMode    = 0;
                restTime_ms    = 10000; // reset rest time preview to default
...
// -------------------------------------------------------
// IR Sensor Interrupt Handler (PE4, falling edge ONLY)
//
// Fires when IR beam is broken = one rep detected.
// Configured for falling edge ONLY, but IR sensors still bounce
// electrically on beam-break, firing the ISR 2-3 times per pass
// in microseconds. A time-based lockout (IR_LOCKOUT_MS) ignores
// any re-trigger within 500 ms of the last valid rep, which is
// faster than any realistic exercise repetition rate.
// Gates on WORKOUT_ACTIVE state, increments repCount, feeds the
// watchdog, and sets pendingInput flag if target reps reached.
...
void IR_Sensor_ISR(void) {
    ...
    // 2. Time-based lockout: ignore re-triggers within IR_LOCKOUT_MS
    // sysTick_ms is incremented every 1 ms by Timer0A_ISR.
    // This filters out electrical bounce on the IR sensor output
    // that causes 2-3 ISR fires per single beam-break event.
    if ((sysTick_ms - lastRepTime_ms) < IR_LOCKOUT_MS) {
        return; // Still within lockout window — ignore
    }
    ...
    // 5. Valid rep — update lockout timestamp
    lastRepTime_ms = sysTick_ms;

    // 6. Feed watchdog: user is actively performing reps, reset the countdown
    WatchdogReloadSet(WATCHDOG0_BASE, WDT_RELOAD);

    // 7. Increment rep counter and print to UART
    repCount++;
    printString(msg_rep);
    printInt(repCount);
    printString(msg_slash);
    ...
    // 8. Brief green LED toggle for per-rep visual feedback
    GPIO_PORTF_DATA_R ^= GREEN_LED_PIN;

    // 9. Check if target reps reached — signal main loop via flag
...
// -------------------------------------------------------
// Button_ISR — handles SW1 (PF4) and SW2 (PF0), falling edge
//
// SW1 (PF4): sets buttonPressed flag for main loop to consume.
//   Used to start workout (SETUP) and restart (WORKOUT_DONE).
// SW2 (PF0): sets confirmPressed flag for main loop to consume.
//   Used during SETUP to toggle encoder between editing reps/sets.
//
// Flag-based approach fixes the double-press bug:
//   Polling readButton() in the main loop sees the button held
//   high across multiple iterations and fires the action twice.
//   The ISR fires exactly once per physical press and sets a flag
//   that the main loop consumes and clears in a single iteration.
// -------------------------------------------------------
void Button_ISR(void) {
    // Read which button was pressed before clearing
    int sw1 = (GPIO_PORTF_DATA_R & BUTTON_PIN) ? 0 : 1;
    int sw2 = (GPIO_PORTF_DATA_R & SW2_PIN)    ? 0 : 1;

    if (sw1) { buttonPressed  = 1; }
    if (sw2) { confirmPressed = 1; }

    // Clear interrupt flags for both pins
    GPIOIntClear(GPIO_PORTF_BASE, BUTTON_PIN | SW2_PIN);
}
...
// -------------------------------------------------------
// readEncoderDelta
// Full quadrature decoder for rotary encoder on PB0 (A) and PB1 (B).
// Returns +1 for one CW detent, -1 for one CCW detent, 0 for no change.
//
// Root cause of the previous bug:
//   The old function only looked at the falling edge of channel A and
//   ignored channel B transitions entirely. A mechanical encoder produces
//   a full quadrature cycle (4 state transitions) per physical detent click.
//   Missing 3 of the 4 transitions meant the function only registered a
//   change roughly once every 4 detents, causing the jumpy/unresponsive
//   behavior. Reading both edges of both channels via a 2-bit state machine
//   catches all 4 transitions and fires exactly once per detent.
//
// How the quadrature state machine works:
//   A and B together form a 2-bit state: (A<<1)|B
//   Valid CW  sequence: 00 -> 10 -> 11 -> 01 -> 00
//   Valid CCW sequence: 00 -> 01 -> 11 -> 10 -> 00
//   The transition table maps (lastState << 2) | currentState to +1, -1, or 0.
//   An accumulator sums partial transitions and only returns a value when a
//   full detent (+4 or -4 quarter-steps) has been accumulated, ensuring one
//   output event per physical click.
// -------------------------------------------------------
int readEncoderDelta(void) {
    // Transition table: index = (prevState << 2) | currState
    // +1 = valid CW quarter step, -1 = valid CCW quarter step, 0 = invalid/no change
    static const int transTable[16] = {
         0, -1, +1,  0,   // prev=00: 00->00, 00->01, 00->10, 00->11
        +1,  0,  0, -1,   // prev=01: 01->00, 01->01, 01->10, 01->11
        -1,  0,  0, +1,   // prev=10: 10->00, 10->01, 10->10, 10->11
         0, +1, -1,  0    // prev=11: 11->00, 11->01, 11->10, 11->11
    };

    static int lastState   = 0;  // previous 2-bit AB state
    static int accumulator = 0;  // sums quarter-steps toward a full detent

    // Read current A and B, form 2-bit state
    int A = (GPIO_PORTB_DATA_R & ENC_A_PIN) ? 1 : 0;
    int B = (GPIO_PORTB_DATA_R & ENC_B_PIN) ? 1 : 0;
    int currState = (A << 1) | B;

    // Look up quarter-step value for this transition
    int step = transTable[(lastState << 2) | currState];
    accumulator += step;
    lastState = currState;

    // A full detent = 4 quarter-steps in the same direction
    // Only return +1 or -1 when a complete click has been accumulated
    if (accumulator >= 4) {
        accumulator = 0;
        return +1; // one full CW detent
    } else if (accumulator <= -4) {
        accumulator = 0;
        return -1; // one full CCW detent
    }
    return 0;
}
...
void watchdog_setup(void) {
    // Watchdog0: ~15 second timeout
    // At 50 MHz: WDT_RELOAD = 50,000,000 * 15 = 750,000,000 ticks = ~15 seconds
